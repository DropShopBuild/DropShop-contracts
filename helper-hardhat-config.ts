...
export interface autoFundCheckParams {
  contractAddr: string;
  networkName: string;
...
export const getNetworkIdFromName = async (networkIdName: string): Promise<null | number> => {
...
export const autoFundCheck = async ({
  ...
  linkTokenAddress,
  additionalMessage,
}: autoFundCheckParams): Promise<boolean> => {
