...
export function shouldBehaveLikeTokenDeploy(): void {
  ...
  it("Should set the right owner", async function () {
    const { Token, sponsor } = this.setup;
    // Expect receives a value, and wraps it in an Assertion object. These
    // objects have a lot of utility methods to assert values.

    // This test expects the owner variable stored in the contract to be equal to our configured owner
    expect(await Token.owner()).to.equal(sponsor.address);
  });

  it("Should assign the total supply of tokens to the owner", async function () {
    const { Token, sponsor } = this.setup;
    const ownerBalance = await Token.balanceOf(sponsor.address);
...
export function shouldBehaveLikeTokenTransaction(): void {
  it("Should transfer tokens between accounts", async function () {
    const { Token, users, sponsor } = this.setup;
    // Transfer 50 tokens from owner to users[0]
    await sponsor.Token.transfer(users[0].address, 50);
  ...
  it("Should fail if sender doesn’t have enough tokens", async function () {
    const { Token, users, sponsor } = this.setup;
    const initialOwnerBalance = await Token.balanceOf(sponsor.address);

    // Try to send 1 token from users[0] (0 tokens) to owner (1000 tokens).
    // `require` will evaluate false and revert the transaction.
    await expect(users[0].Token.transfer(sponsor.address, 1)).to.be.revertedWith("Not enough tokens");

    // Owner balance shouldn't have changed.
    expect(await Token.balanceOf(sponsor.address)).to.equal(initialOwnerBalance);
  });
  ...
  it("Should update balances after transfers", async function () {
    ...
    await sponsor.Token.transfer(users[0].address, 100);

    // Transfer another 50 tokens from owner to users[1].
    await sponsor.Token.transfer(users[1].address, 50);

    // Check balances.
    const finalOwnerBalance = await Token.balanceOf(sponsor.address);
